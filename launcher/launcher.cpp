...
// Log function
//-----------------------------------------------------------------------------
void COM_TimestampedLog(const tchar* format, ...) {
...
SpewRetval_t LauncherDefaultSpewFunc(SpewType_t spewType, char const* pMsg)
{
	...
	case SPEW_MESSAGE:
		if (enableHardSpew)
			::MessageBox(NULL, pMsg, "Message", MB_OK | MB_SYSTEMMODAL);
...
void TryToLoadSteamOverlayDLL()
{
	...
	if (bSteamActive)
	{
		...
		Q_strcat(rgchSteamPath, "\\GameOverlayRenderer.dll", ARRAYSIZE(rgchSteamPath));
